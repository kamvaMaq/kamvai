...
import { AlertCircle, Archive, BarChart3, Check, ChevronRight, Copy, CreditCard, Download, ImageIcon, Link2, Loader2, LockKeyhole, Mail, Mic, Moon, PanelLeft, Plus, ShieldCheck, Share2, Sparkles, Sun, WandSparkles, X } from "lucide-react";
...
export default function Home() {
  ...
  return (
    <div className="min-h-screen bg-[radial-gradient(circle_at_80%_0%,rgba(221,167,70,.14),transparent_26rem),radial-gradient(circle_at_0%_25%,rgba(40,141,132,.11),transparent_29rem)] text-foreground">
      ...
      <main className="container grid gap-8 py-8 lg:grid-cols-[236px_minmax(0,1fr)] lg:py-10">
        <aside className="hidden lg:block"><div className="sticky top-28"><button onClick={() => { setBrief(""); setRefinement(""); setActiveDraftId(null); }} className="flex w-full items-center gap-3 rounded-xl bg-[#244F49] px-4 py-3 text-sm font-semibold text-white shadow-[0_12px_26px_rgba(36,79,73,.18)]"><Plus size={16} /> {t("newDraft")}</button><div className="mt-7"><p className="px-3 text-[11px] font-bold uppercase tracking-[.16em] text-muted-foreground">{t("yourLibrary")}</p><div className="mt-2 space-y-1">{draftsQuery.data?.slice(0, 6).map(draft => <button key={draft.id} onClick={() => setActiveDraftId(draft.id)} className={`w-full rounded-xl px-3 py-2.5 text-left text-sm transition ${activeDraft?.id === draft.id ? "bg-[#E7EFEA] text-[#1B453F] dark:bg-[#254840] dark:text-white" : "text-muted-foreground hover:bg-muted"}`}><span className="block truncate font-medium">{draft.title}</span><span className="mt-1 block text-[11px] capitalize opacity-70">{draft.kind} · {new Date(draft.updatedAt).toLocaleDateString()}</span></button>)}</div></div>{isAuthenticated && <div className="mt-7 border-t border-border pt-5"><p className="px-3 text-[11px] font-bold uppercase tracking-[.16em] text-muted-foreground">Workspace tools</p><div className="mt-2 space-y-1"><Button variant="ghost" className="w-full justify-start rounded-xl text-sm" onClick={() => setWorkspacePanel("analytics")}><BarChart3 size={16} />Contribution & goals</Button><PromptLibrary onApply={template => { setKind(template.kind); setBrief(template.prompt); setRefinement(""); }} /><Button variant="ghost" className="w-full justify-start rounded-xl text-sm" onClick={() => setWorkspacePanel("sharing")}><Share2 size={16} />Shared prompt reach</Button><Button variant="ghost" className="w-full justify-start rounded-xl text-sm" onClick={() => setWorkspacePanel("payments")}><CreditCard size={16} />Payments & passes</Button><Button variant="ghost" className="w-full justify-start rounded-xl text-sm" onClick={() => setWorkspacePanel("privacy")}><ShieldCheck size={16} />Privacy & data</Button></div><Button size="sm" className="mt-4 w-full rounded-xl bg-[#244F49] text-white hover:bg-[#173b36]" onClick={() => openPayment()}><Sparkles size={15} />Explore passes</Button></div>}<div className="mt-8 rounded-2xl border border-[#D8D0BA] bg-[#F6F0E2] p-4 dark:border-[#59635D] dark:bg-[#27332E]"><div className="flex items-center gap-2 text-[#244F49] dark:text-[#CFE7D4]"><LockKeyhole size={15} /><p className="text-xs font-bold">{t("paymentSafe")}</p></div><p className="mt-2 text-xs leading-5 text-muted-foreground">{t("paymentSafeBody")}</p></div></div></aside>

        <section className="min-w-0">
          <div className="mb-7 max-w-2xl"><p className="text-xs font-bold uppercase tracking-[.18em] text-[#B36935]">{isAuthenticated ? `Welcome back, ${user?.name?.split(" ")[0] ?? "creator"}` : "KAMVAI STUDIO"}</p><h1 className="mt-3 font-display text-4xl leading-[1.02] tracking-tight sm:text-5xl">{t("welcome")}</h1><p className="mt-4 max-w-xl text-base leading-7 text-muted-foreground">{t("welcomeBody")}</p>{isAuthenticated && <div className="mt-5 flex gap-2 overflow-x-auto pb-1 lg:hidden"><Button size="sm" variant="outline" className="shrink-0 rounded-full" onClick={() => setWorkspacePanel("analytics")}><BarChart3 size={14} />Insights</Button><PromptLibrary onApply={template => { setKind(template.kind); setBrief(template.prompt); setRefinement(""); }} triggerClassName="mb-0 h-8 shrink-0 px-3 text-xs" /><Button size="sm" variant="outline" className="shrink-0 rounded-full" onClick={() => setWorkspacePanel("sharing")}><Share2 size={14} />Sharing</Button><Button size="sm" variant="outline" className="shrink-0 rounded-full" onClick={() => setWorkspacePanel("payments")}><CreditCard size={14} />Payments</Button><Button size="sm" variant="outline" className="shrink-0 rounded-full" onClick={() => setWorkspacePanel("privacy")}><ShieldCheck size={14} />Privacy</Button><Button size="sm" className="shrink-0 rounded-full bg-[#244F49] text-white hover:bg-[#173b36]" onClick={() => openPayment()}><Sparkles size={14} />Get a pass</Button></div>}</div>
        ...
        <aside className="hidden"><div className="rounded-[1.4rem] border border-border bg-card p-5 shadow-[0_14px_42px_rgba(46,41,30,.05)]"><div className="flex items-start justify-between"><div><p className="text-[11px] font-bold uppercase tracking-[.16em] text-[#B36935]">{t("freeAllowance")}</p><p className="mt-1 font-display text-2xl">{usageQuery.data?.unlimited ? t("unlimited") : isAuthenticated ? `${usageQuery.data?.remaining ?? 0} ${t("remaining")}` : "—"}</p></div><div className="grid size-10 place-items-center rounded-xl bg-[#EAF3EE] text-[#244F49] dark:bg-[#1E3E39] dark:text-[#C7F0DD]"><Sparkles size={17} /></div></div><div className="mt-4 h-1.5 overflow-hidden rounded-full bg-muted"><div className="h-full rounded-full bg-[#5E8D80]" style={{ width: `${usageQuery.data?.unlimited ? 100 : ((usageQuery.data?.used ?? 0) / (usageQuery.data?.limit ?? 5)) * 100}%` }} /></div><p className="mt-3 text-xs leading-5 text-muted-foreground">{usageQuery.data?.unlimited ? "Your pass is active." : t("reset")}</p></div>
