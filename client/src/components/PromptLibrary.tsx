...
import { BookOpen, Check, Heart, Pencil, Plus, RefreshCw, Search, Sparkles, Trash2 } from "lucide-react";
...
export function PromptLibrary({ onApply }: { onApply: (template: { prompt: string; kind: PromptKind; title: string }) => void }) {
  ...
  const input = useMemo(() => ({ query: query || undefined, kind: kind === "all" ? undefined : kind, locale, favoritesOnly: view === "library" && favoritesOnly }), [query, kind, locale, favoritesOnly, view]);
  ...
  const createPrompt = trpc.promptLibrary.create.useMutation({
    onSuccess: () => { setEditor(blankPrompt); invalidate(); toast.success("Custom prompt saved to My Prompts."); },
    onError: error => toast.error(error.message || "We could not save that prompt."),
  });
  const updatePrompt = trpc.promptLibrary.update.useMutation({
    onSuccess: () => { setEditor(blankPrompt); setEditingId(null); invalidate(); toast.success("Custom prompt updated."); },
    onError: error => toast.error(error.message || "We could not update that prompt."),
  });
  const removePrompt = trpc.promptLibrary.remove.useMutation({
    onSuccess: () => { if (editingId) { setEditingId(null); setEditor(blankPrompt); } invalidate(); toast.success("Custom prompt removed."); },
    onError: error => toast.error(error.message || "We could not remove that prompt."),
  });
  ...
  function edit(prompt: typeof prompts[number]) {
  ...
  const promptCards = (items: typeof prompts, management = false) => items.length ? (
    ...
    <div className="grid place-items-center py-12 text-center"><div><div className="mx-auto grid size-11 place-items-center rounded-2xl bg-muted"><Check size={19} className="text-muted-foreground" /></div><p className="mt-3 font-semibold">{management ? "No custom prompts yet" : favoritesOnly ? "No favourites yet" : "No prompt matches yet"}</p><p className="mt-1 text-sm text-muted-foreground">{management ? "Create a reusable template to keep your best ideas close." : "Try a different format or a broader search."}</p></div></div>
  );
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogContent className="max-h-[86vh] overflow-y-auto sm:max-w-4xl">
        <DialogHeader>
          ...
          <DialogDescription>{locale === "zu" ? "Qala ngemiyalelo ebhalwe ngesiZulu, noma ulondoloze awakho amathuluzi." : locale === "xh" ? "Qala ngezikhokelo ezibhalwe ngesiXhosa, okanye ugcine ezakho iitemplate." : "Browse reliable starters, save favourites, and manage your own reusable templates."}</DialogDescription>
        </DialogHeader>
        <div className="mt-2 flex flex-wrap gap-2 border-b border-border pb-4">
          <Button size="sm" variant={view === "library" ? "default" : "outline"} className={view === "library" ? "rounded-xl bg-[#244F49] text-white hover:bg-[#173b36]" : "rounded-xl"} onClick={() => setView("library")}><BookOpen size={14} />Starter library</Button>
          <Button size="sm" variant={view === "mine" ? "default" : "outline"} className={view === "mine" ? "rounded-xl bg-[#244F49] text-white hover:bg-[#173b36]" : "rounded-xl"} onClick={() => setView("mine")}><Plus size={14} />My Prompts</Button>
        </div>
        {view === "library" ? <>
          <div className="mt-4 flex flex-col gap-3 sm:flex-row">
            <div className="relative flex-1"><Search className="absolute left-3 top-1/2 -translate-y-1/2 text-muted-foreground" size={16} /><Input value={query} onChange={event => setQuery(event.target.value)} placeholder="Search prompts, categories, or outcomes" className="pl-9" /></div>
            <Select value={kind} onValueChange={value => setKind(value as "all" | PromptKind)}><SelectTrigger className="w-full sm:w-36"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="all">All formats</SelectItem><SelectItem value="blog">Blog</SelectItem><SelectItem value="email">Email</SelectItem><SelectItem value="code">Code</SelectItem><SelectItem value="image">Image</SelectItem></SelectContent></Select>
            <Button variant={favoritesOnly ? "default" : "outline"} aria-pressed={favoritesOnly} className={favoritesOnly ? "rounded-xl bg-[#C66A48] text-white hover:bg-[#A95137]" : "rounded-xl"} onClick={() => setFavoritesOnly(value => !value)}><Heart size={15} fill={favoritesOnly ? "currentColor" : "none"} />Favourites</Button>
          </div>
          {libraryQuery.isLoading ? <div className="grid gap-3 py-6 sm:grid-cols-2">{Array.from({ length: 4 }).map((_, index) => <div key={index} className="h-44 animate-pulse rounded-2xl bg-muted" />)}</div> : libraryQuery.isError ? <div className="grid place-items-center py-14 text-center"><div><div className="mx-auto grid size-11 place-items-center rounded-2xl bg-[#FBEDE6] text-[#B6503A] dark:bg-[#4A2A22]"><RefreshCw size={19} /></div><p className="mt-3 font-semibold">Your prompt library could not load</p><p className="mt-1 max-w-sm text-sm text-muted-foreground">Your current brief is safe. Please try again in a moment.</p><Button variant="outline" size="sm" className="mt-4 rounded-xl" onClick={() => libraryQuery.refetch()}><RefreshCw size={14} />Try again</Button></div></div> : promptCards(libraryPrompts)}
        </> : <>
          <form onSubmit={submitPrompt} className="mt-4 rounded-2xl border border-[#C7D7CF] bg-[#F1F7F2] p-4 dark:border-[#426357] dark:bg-[#20382F]">
            <div className="flex flex-wrap items-center justify-between gap-2"><div><h3 className="font-semibold">{editingId ? "Edit custom prompt" : "Create a custom prompt"}</h3><p className="mt-1 text-sm text-muted-foreground">Only you can edit or delete these templates.</p></div>{editingId && <Button type="button" size="sm" variant="ghost" onClick={() => { setEditingId(null); setEditor(blankPrompt); }}>Cancel edit</Button>}</div>
            <div className="mt-4 grid gap-3 sm:grid-cols-2"><Input value={editor.title} onChange={event => setEditor(current => ({ ...current, title: event.target.value }))} placeholder="Prompt title" maxLength={120} required /><Input value={editor.category} onChange={event => setEditor(current => ({ ...current, category: event.target.value }))} placeholder="Category, e.g. Sales" maxLength={64} required /><Select value={editor.kind} onValueChange={value => setEditor(current => ({ ...current, kind: value as PromptKind }))}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="blog">Blog</SelectItem><SelectItem value="email">Email</SelectItem><SelectItem value="code">Code</SelectItem><SelectItem value="image">Image</SelectItem></SelectContent></Select></div>
            <Textarea value={editor.body} onChange={event => setEditor(current => ({ ...current, body: event.target.value }))} placeholder="Write the reusable instruction you want Kamvai to follow…" className="mt-3 min-h-28 bg-background" maxLength={6000} required />
            <div className="mt-3 flex justify-end"><Button type="submit" disabled={createPrompt.isPending || updatePrompt.isPending} className="rounded-xl bg-[#244F49] text-white hover:bg-[#173b36]"><Plus size={15} />{editingId ? "Save changes" : "Save prompt"}</Button></div>
          </form>
          {libraryQuery.isLoading ? <div className="mt-5 h-40 animate-pulse rounded-2xl bg-muted" /> : libraryQuery.isError ? <div className="mt-5 rounded-2xl border border-destructive/30 p-5 text-sm text-muted-foreground">Your custom prompts could not load. <Button variant="link" className="h-auto p-0" onClick={() => libraryQuery.refetch()}>Try again</Button></div> : promptCards(myPrompts, true)}
        </>}
      </DialogContent>
    </Dialog>
  </>;
