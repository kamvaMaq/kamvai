...
import { FileImage, FileText, FileUp, Loader2, Paperclip, Trash2, UploadCloud } from "lucide-react";
...
export function DocumentUploader() {
  ...
  const allowanceQuery = trpc.documents.allowance.useQuery();
  ...
  return <section className="mt-7 overflow-hidden rounded-[1.5rem] border border-[#C7D7CF] bg-[#F1F7F2] shadow-[0_22px_70px_rgba(46,41,30,.06)] dark:border-[#426357] dark:bg-[#20382F]">
    <div className="flex flex-wrap items-start justify-between gap-4 border-b border-[#D7E4DB] px-5 py-5 dark:border-[#426357] sm:px-6">
      <div><p className="text-[11px] font-bold uppercase tracking-[.16em] text-[#B36935]">Document workspace</p><h2 className="mt-1 font-display text-2xl tracking-tight">Attach supporting files</h2><p className="mt-1 text-sm text-muted-foreground">PDF, Word, JPG, PNG, or WebP up to 10 MB each.</p></div>
      <div className="rounded-2xl bg-[#DDECE1] px-3 py-2 text-right dark:bg-[#315345]"><p className="text-lg font-bold text-[#244F49] dark:text-[#D4F5E0]">{allowanceQuery.isLoading ? "…" : remaining}</p><p className="text-[10px] font-bold uppercase tracking-[.12em] text-[#426357] dark:text-[#C7F0DD]">of 3 uploads left</p></div>
    </div>
    <div className="p-5 sm:p-6"><input ref={inputRef} type="file" className="sr-only" accept=".pdf,.doc,.docx,.jpg,.jpeg,.png,.webp,application/pdf,application/msword,application/vnd.openxmlformats-officedocument.wordprocessingml.document,image/jpeg,image/png,image/webp" onChange={handleFileSelection} />
      ...
      {documentsQuery.isLoading ? <div className="mt-5 space-y-2">{Array.from({ length: 2 }).map((_, index) => <div key={index} className="h-14 animate-pulse rounded-xl bg-background/60" />)}</div> : documentsQuery.isError ? <div className="mt-5 rounded-xl border border-destructive/30 bg-background/50 p-4 text-sm text-muted-foreground">Your attached files could not load. <Button variant="link" className="h-auto p-0" onClick={() => documentsQuery.refetch()}>Try again</Button></div> : documents.length ? <ul className="mt-5 divide-y divide-[#D7E4DB] rounded-xl border border-[#D7E4DB] bg-background/50 dark:divide-[#426357] dark:border-[#426357]">{documents.map(document => <li key={document.id} className="flex items-center gap-3 px-3 py-3"><div className="grid size-9 shrink-0 place-items-center rounded-xl bg-[#DDECE1] text-[#244F49] dark:bg-[#315345] dark:text-[#D4F5E0]"><DocumentIcon mimeType={document.mimeType} /></div><div className="min-w-0 flex-1"><p className="truncate text-sm font-semibold">{document.fileName}</p><p className="mt-0.5 text-xs text-muted-foreground">{formatBytes(document.sizeBytes)} · {new Date(document.createdAt).toLocaleDateString()}</p></div><Button size="icon" variant="ghost" aria-label={`Open ${document.fileName}`} disabled={downloadingId === document.id} onClick={() => void download(document.id)}>{downloadingId === document.id ? <Loader2 className="animate-spin" size={16} /> : <Paperclip size={16} />}</Button><Button size="icon" variant="ghost" aria-label={`Remove ${document.fileName}`} disabled={remove.isPending} className="text-destructive hover:text-destructive" onClick={() => remove.mutate({ documentId: document.id })}><Trash2 size={16} /></Button></li>)}</ul> : <div className="mt-5 grid place-items-center rounded-xl border border-dashed border-[#C7D7CF] bg-background/40 p-7 text-center dark:border-[#426357]"><FileUp size={20} className="text-muted-foreground" /><p className="mt-3 font-semibold">No documents attached yet</p><p className="mt-1 max-w-sm text-sm text-muted-foreground">Upload a reference document or image to keep it available in your private workspace.</p></div>}
    </div>
  </section>;
}
